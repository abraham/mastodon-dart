...
------------ | ------------- | ------------- | -------------
**automatic** | [**List&lt;FeatureApprovalAutomaticEnum&gt;**](FeatureApprovalAutomaticEnum.md) | Describes who is expected to be able to feature the account in a Collection and have this authorized automatically. An empty list means that nobody is expected to be able to feature this account with automatic approval. Other values may be added in the future, so unknown values should be treated as `unsupported_policy`. | 
